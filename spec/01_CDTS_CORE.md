# CDTS Core v0.1-draft

## Status and scope

This document is normative only for CDTS trace representation and admissibility. RFC 2119 terms are used in their ordinary standards sense.
...
**CDTS-CORE-005** The exact fixed revision of each owner represented by a record reference MUST be listed. Review Protocol MUST be pinned as `source_policy`.

**CDTS-CORE-006** ARB MAY be listed only as `analytical_mapping`; it is not a normative dependency.

**CDTS-CORE-007** MPAA, BEC, and PCA retain sole authority for their record formats, validation, terms, and conclusions. CDTS MUST NOT represent an external conclusion as a CDTS-owned conclusion or translate, recompute, compare, or select it. The reference validator rejects dedicated conclusion fields but does not semantically classify free-text values; semantic compliance of such values is `NOT_EVALUATED` and requires separate review.

**CDTS-CORE-008** PCA's historical Linkage Record does not become a CDTS object. The pinned PCA Core has no current normative Linkage Record. CDTS defines a separate `linkage_assertion`.

**CDTS-CORE-009** Link endpoints and evidence references MUST resolve to local `ref_id` values. Self-links are forbidden.

**CDTS-CORE-010** Any absence represented by CDTS MUST use a typed state: `not_applicable`, `not_observed`, `not_produced`, `unavailable`, or `undetermined`, with a reason. CDTS v0.1 does not define an expected-record universe and does not evaluate record-set completeness.
...
**CDTS-CORE-014** Trace provenance MUST identify its producer with a qualified identifier and the role `coordination_layer`. Producer-authored validation fields are forbidden. External record producer identity and distinctness are `NOT_EVALUATED` unless supplied by a separate addressable attestation.
...
**CDTS-CORE-016** Implementations MUST reject duplicate JSON keys, non-finite numbers, invalid UTF-8, unknown normative tokens, and unsupported normative schema keywords.
...
The root consists of version and revision identity; `trace_scope`; fixed `source_revisions`; qualified `record_refs`; typed `absences`; CDTS-owned `linkage_assertions`; `conflicts`; `unresolved`; producer `provenance`; and `amendments`. The JSON Schema is structural. Cross-reference, revision, temporal, ownership, conflict-policy, amendment, and derived-status checks belong to the validator.
